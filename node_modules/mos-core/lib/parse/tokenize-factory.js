...
exports.default = tokenizeFactory;
...
var ERR_INCORRECTLY_EATEN = 'Incorrectly eaten value: please report this ' + 'warning on http://git.io/vg5Ft';
...
function tokenizeFactory(parser, type) {
  ...
  function tokenize(value, location) {
    ...
    if (!value) {
      return _promise2.default.resolve(tokens);
    }
    ...
    function add(node, parent) {
      ...
      if (prev && node.type === prev.type && node.type in _utilities.MERGEABLE_NODES && (0, _utilities.mergeable)(prev) && (0, _utilities.mergeable)(node)) {
        node = _utilities.MERGEABLE_NODES[node.type].call(parser, prev, node);
      }
    ...
    function eat(subvalue) {
      ...
      function apply(node, parent) {
        return node instanceof _promise2.default ? node.then(updatePos) : _promise2.default.resolve(updatePos(node));
       ...
       */
      function reset() {
        return apply.apply(undefined, arguments).then(function (node) {
          line = current.line;
    ...
    function process() {
      if (value) {
        return matchMethods(tokenizers.slice()).then(function () {
          return process();
        });
      }
      ...
      function matchMethods(tokenizers) {
        ...
        if (tokenizer && tokenizer.func && (!tokenizer.func.onlyAtStart || parser.state.atStart) && (!tokenizer.func.onlyAtTop || parser.state.atTop) && (!tokenizer.func.notInBlockquote || !parser.state.inBlockquote) && (!tokenizer.func.notInLink || !parser.state.inLink)) {
          var _ret = function () {
            var valueLength = value.length;

            return {
              v: (0, _runAsync2.default)(tokenizer.func)((0, _assign2.default)({}, parser, { eat: eat }), value).then(function () {
