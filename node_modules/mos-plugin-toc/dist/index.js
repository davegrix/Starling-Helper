...
function isOpeningHeading(node, depth, expression) {
  return depth === null && node && node.type === HEADING && expression.test(toString(node));
}
...
function isClosingHeading(node, depth) {
  return node && (depth && node.type === HEADING && node.depth <= depth || node.type === 'markdownScript');
}
...
function insert(node, parent) {
  ...
  if (node.depth === 1) {
    ...
    item.children.push(m.paragraph([m.link({ title: null, url: '#' + node.id }, [node.value])]));
...
function toString(node) {
  return (valueOf(node) || node.children && node.children.filter(function (child) {
    return child.type !== 'html';
  }).map(toString).join('') || '').trim();
}
...
function plugin(mos, md) {
  ...
  mos.compile.pre(function (next, node, opts) {
    ...
    node.children = [].concat(node.children.slice(0, result.index), contents(result.map), node.children.slice(result.index));
  ...
  function search(root, expression, maxDepth) {
    ...
    var depth = null;
    var map = [];
    var headingIndex = void 0;
    var closingIndex = void 0;
